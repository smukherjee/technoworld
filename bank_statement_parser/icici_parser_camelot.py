...
class ICICIParserCamelot(BankStatementParser):
    ...
    def extract_transactions(self) -> List[Dict[str, Any]]:
        """Extract transactions from PDF using Camelot."""
        self.logger.info(f"Starting to parse ICICI statement: {self.pdf_path}")
    ...
    def parse_transaction_line(self, line: str) -> Optional[Dict[str, Any]]:
        """Parse a single transaction line - required by base class but not used in Camelot approach."""
        # This method is required by the abstract base class but not used in our table-based approach
        return None
